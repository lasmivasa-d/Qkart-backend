 ...
 */
 const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
  ...
  return jwt.sign({sub: userId , type: type , exp: expires} , secret); 
};
/**
 * Generate auth token
 * - Generate jwt token
 ...
 */
 const generateAuthTokens = async (user) => {
  ...
  const expiresInSeconds = Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60;
  // console.log(expiresInSeconds);
  const token = generateToken(userId, expiresInSeconds, type);
